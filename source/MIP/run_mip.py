...
def build_model(n, optimization, simmetry_br):
    ...
    for i, j in pairs:
        if i < j:
            problem += (
                lpSum(
                    matches[(i, j)][w][p] + matches[(j, i)][w][p]
                    for w in WEEKS
                    for p in PERIODS
                )
                == games_per_pair,
    ...
    for t in TEAMS:
        for w in WEEKS:
            problem += (
                lpSum(
                    matches[(t, j)][w][p] + matches[(j, t)][w][p]
                    for j in TEAMS
                    if j != t
                    for p in PERIODS
                )
    ...
    for w in WEEKS:
        for p in PERIODS:
            problem += (
                lpSum(matches[(i, j)][w][p] for (i, j) in pairs) == games_per_slot,
                f"4c_{w}_{p}",
            )
    ...
    if simmetry_br != "noSym":
        ...
        if simmetry_br == "fixWeek2":
            ...
            util_list = [x for x in range(1, n + 1) if x % 2 != 0] + [
    ...
    if optimization != "feasible":
        if optimization == "homeAwayDiff1":
            home_away_diff = LpVariable.dicts(
                "HomeAwayDiff", TEAMS, lowBound=1, upBound=n - 1, cat="Integer"
            )
            for t in TEAMS:
                home_games = lpSum(
                    matches[(t, j)][w][p]
        ...
        if optimization == "homeAwayDiff0":
            home_away_diff = LpVariable.dicts(
                "HomeAwayDiff", TEAMS, lowBound=0, upBound=n - 2, cat="Integer"
            )
            for t in TEAMS:
                home_games = lpSum(
                    matches[(t, j)][w][p]
                ...
                problem += (
                    home_away_diff[t] + 1 >= home_games - away_games,
                    f"diff1_{t}",
                )
                problem += (
                    home_away_diff[t] + 1 >= away_games - home_games,
                    f"diff2_{t}",
                )
...
def run_single_instance(
    n, solver_type, opt, sym, time_limit, solver_verbose, json_path, json_name
):
    """Run a single instance with specified parameters"""
    assert n >= 2
    assert n % 2 == 0
    ...
    if solver_type == "cbc":
        solver = PULP_CBC_CMD(
            msg=solver_verbose, timeLimit=time_limit, warmStart=warmStart
        )
    ...
    print(
        f"Completed: n={n}, solver={solver_type}, opt={opt}, sym={sym}, time={runtime}s, optimal={is_optimal}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="single",
        choices=["single", "fix_n", "all"],
        help="Run mode: single instance, all configs for fixed n, or all configs for all n",
    )
    parser.add_argument(
        "--n",
        type=int,
        choices=range(6, 15, 2),
        help="Number of teams (required for single and fix_n modes)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="cbc",
        choices=["cbc", "highs", "sciPy"],
        help="Solver type (for single mode)",
    )
    parser.add_argument(
        "--opt",
        type=str,
        default="feasible",
        choices=["homeAwayDiff0", "homeAwayDiff1", "feasible"],
        help="Optimization tecnique (for single mode)",
    )
    parser.add_argument(
        "--sym",
        type=str,
        default="noSym",
        choices=["circleMethod", "fixWeek1", "fixWeek2", "noSym"],
    ...
    # Define all possible values for batch modes
    all_solver_types = ["cbc", "highs", "sciPy"]
    all_opts = ["homeAwayDiff0", "homeAwayDiff1", "feasible"]
    all_syms = ["circleMethod", "fixWeek1", "fixWeek2", "noSym"]
    all_n_values = [6, 8, 10, 12, 14]

    if mode == "single":
        # Mode 1: Run a specific instance
        if args.n is None:
            parser.error("--n is required for single mode")
        ...
        # Mode 2: Run all instances but fixing n
        if args.n is None:
            parser.error("--n is required for fix_n mode")

        print(f"\n=== Running all configurations for n={args.n} ===\n")

        for solver_type, opt, sym in itertools.product(
            all_solver_types, all_opts, all_syms
        ):
            run_single_instance(
                n=args.n,
                ...
                solver_verbose=solver_verbose,
                json_path=json_path,
                json_name=json_name if json_name else f"{args.n}.json",
            )

    elif mode == "all":
        # Mode 3: Run all instances for all n
        print("\n=== Running all configurations for all n values ===\n")

        for n in all_n_values:
            print(f"\n--- Starting runs for n={n} ---")
            for solver_type, opt, sym in itertools.product(
                all_solver_types, all_opts, all_syms
            ):
                run_single_instance(
                    ...
                    solver_verbose=solver_verbose,
                    json_path=json_path,
                    json_name=json_name if json_name else f"{n}.json",
                )
